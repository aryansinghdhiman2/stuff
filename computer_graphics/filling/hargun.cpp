...
void mouseCallback(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && !drawingPolygon) {
        // float normalizedX = (2.0 * x / WIDTH - 1.0);
        // float normalizedY = (-2.0 * y / HEIGHT + 1.0);
...
void drawPolygon() {
    ...
    for (size_t i = 0; i < vertices.size(); ++i) {
        ...
        storeEdgeInTable((int)vertex.x, (int)vertex.y, (int)nextVertex.x, (int)nextVertex.y);
    }
