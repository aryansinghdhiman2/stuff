...
/**
 * Used to compute Hash of a State
 */
struct HashState
{
...
/**
 * @brief Used to represent a State in NFA
 */
struct State
{
    int id; /**< The id of the state */
    bool is_accepting; /**<  If the state is an accepted state of the NFA*/
    std::optional<std::string> combined_id; /**< Id used when state is made from combining many states */
    std::optional<TransitionSet> combined_set;/**< Set of states used to make this state */
    std::unordered_map<char,TransitionSet> transitions;/**< Transitions of this set */

    /**
     * Constructor for the State class
     * @param identifier Used as identifier for the class
     * @param accept Used to specify if it is an acceptable state
    */
     ...
     * 
     * @param alphabet Alphabet on which transition happens
     * @param s State on which it transitions to
     */
     ...
     * 
     * Compares based on State::combined_id if exists otherwise on State::id
     */
    bool operator==(const State& other)
    {
...
/**
 * @brief Used to represent an Automata
 */
struct Automata
{
    StateSharedPointer initial_state; /**< Entry State of the FA */
    StateSharedPointer final_state; /**< Final State of NFA.
                                         Only used with NFA
                                     */
    
    /**
     * Contains all the States in the Finite Automata
     */
    ...
    /**
     * Creates an Automata having a transition on tran
     * 
     * @param tran Character on which Automata has a transition
     */
    Automata(char tran)
    {
     ...
     * Copy Constructor
     * 
     * @param other The Automata to copy
     */
...
/**
 * @brief Concatenates two Finite Automatas
 * 
 * Concatenates two Automatas and returns a new one
 * 
 * @param first First Automata
 * @param second Second Automata 
 * @return The concatenated Automata
 */
