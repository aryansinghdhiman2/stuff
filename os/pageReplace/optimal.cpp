...
class requestBuffer{
    ...
    int getLateUsed(const vector<int>& requestQueue,const int& currentIndex){
        ...
        for(size_t i=0;i<this->bufferSize;i++){
            for(size_t j=currentIndex+1;j<requestQueue.size();j++){
                if(requestQueue.at(j)==this->internalBuffer.at(i)){
                    dist.at(i)=j-currentIndex;
                    break;
                }
            }
        }
        auto usedLater = max_element(dist.begin(),dist.end());
...
int optimal(const vector<int>& requestQueue,const size_t& bufferSize){
...
int main(){
    ...
    int count=optimal(requests,bufferSize);        
