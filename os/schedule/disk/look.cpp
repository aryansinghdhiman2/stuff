...
int look(vector<int>& diskQueue,int seekHeadPos,int lastCylinder){
    int totalDistance=0;
    int prevPos=0;
    ...
    while(not diskQueue.empty()){
        if(seekHeadPos>prevPos){
            ...
            totalDistance+=abs(seekHeadPos-diskQueue.back());
            prevPos=seekHeadPos;
            seekHeadPos=diskQueue.back();
        }
        else{
            int j=0;
            ...
            totalDistance+=abs(seekHeadPos-diskQueue.front());
            prevPos=seekHeadPos;
...
int main(){
    ...
    auto a = look(diskQueue,seekHeadPos,lastCylinderNumber);
