...
# def ratePosition(board:Board,O_X:int) -> int:
    ...
    if(numberOfposswin(board,O_X) > 0):
        rating+= (numberOfposswin(board,O_X) * (10))
...
def computerMove(board:Board,turn:int,depth:int):
    ...
    result = minMax(board,player,depth)
...
if(__name__ =="__main__"):
    # minMax.counter = 0
    # board:Board = [2,2,2,2,O,X,2,2,2]
    # res = minMax(board,X,1)
    # exit()
    ...
    if(moveFirst != 0):
        while(True):
            ...
            if(checkWin(board,X)):
                ...
                break
            displayBoard(board)
            print(f"Number of Calls {minMax.counter}")
            minMax.counter = 0
            print("Your Turn")
            turn+=1
            if(turn > 9): break
